...
export default function AdminCuponsCadastrarPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        showToast(`Erro ao cadastrar cupom: ${error.message}`, 'error');
        return;
      }
      showToast('Cupom cadastrado com sucesso!', 'success');
      navigate('/paineladmin/cupons/listar');
    } catch (err: any) {
      console.error('Erro inesperado ao cadastrar cupom:', err);
      showToast('Erro inesperado ao cadastrar cupom', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Cadastrar Cupom</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Preencha os dados para criar um novo cupom</p>
        </div>
